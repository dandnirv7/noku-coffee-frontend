...
interface SearchProductErrorProps {
  reset: () => void;
  message?: string;
}

export function SearchProductError({
  reset,
  message,
}: SearchProductErrorProps) {
  return (
    <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed rounded-[2rem] bg-muted/5 border-muted-foreground/10 animate-fade-in-up">
      ...
      <div className="mb-10 space-y-3 max-w-md">
        <h3 className="text-2xl font-black tracking-tight uppercase text-slate-900">
          Etalase Produk Sedang Disusun
        </h3>
        <p className="text-sm leading-relaxed text-muted-foreground">
          {message ||
            "Kami gagal menampilkan produk yang kamu cari. Tenang — biji kopi, peralatan, aksesoris dan bundle favorit masih tersedia di rak lainnya."}
        </p>
      </div>
